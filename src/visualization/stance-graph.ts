...
import { Stance, PlannedOperation } from '../types/index.js';
 ...
 */
export function generateStanceGraph(stance: Stance): GraphData {
  const nodes: VisualNode[] = [];
  const links: VisualLink[] = [];

  // Central stance node
  const stanceNode: VisualNode = {
    id: 'stance-current',
    type: 'stance',
  ...
  const sentienceNode: VisualNode = {
    id: 'sentience',
    type: 'stance',
    label: 'Sentience',
 ...
 */
export function generateTransformationGraph(
  ...
  for (let i = 0; i < recentHistory.length; i++) {
    ...
    const transformNode: VisualNode = {
      id: `transform-${i}`,
      type: 'stance',
      ...
      metadata: {
        scores: entry.scores,
        operators: entry.operators.map((o: PlannedOperation) => o.name),
    ...
    if (i > 0) {
      links.push({
        ...
        strength: 0.8,
        type: 'transformation',
        label: entry.operators.map((o: PlannedOperation) => o.name).join(', ')
      });
    }
  ...
  return {
    ...
    metadata: {
      ...
      totalDrift: recentHistory.reduce((sum, h) => {
        const drift = Object.values(h.stanceAfter.values).reduce((a: number, b: number) => a + b, 0) -
                     Object.values(h.stanceBefore.values).reduce((a: number, b: number) => a + b, 0);
 ...
 */
export function generateVisualizationHTML(graphData: GraphData, title: string = 'METAMORPH Stance'): string {
 ...
 */
export function exportGraphJSON(graphData: GraphData): string {
...
class VisualizationManager {
  private lastGraph: GraphData | null = null;
