...
export interface MultiModalMemory {
  ...
  createdAt: Date;
  importance: number;
  relatedMemories: string[];  // IDs of related text memories
}
...
export interface ImageAnalysis {
  ...
  stanceRelevance?: string;  // How it relates to current stance
}
...
export interface MultiModalSearchOptions {
  ...
  limit?: number;
  similarTo?: string;  // Text to find visually similar images
}
...
class MultiModalMemoryManager {
  private config: MultiModalConfig = DEFAULT_CONFIG;
  private memories: Map<string, MultiModalMemory> = new Map();
   ...
   * Calculate file hash
   */
  private hashFile(data: Buffer): string {
   ...
   */
  private detectFormat(data: Buffer): ImageFormat | null {
    // PNG signature
...
how might this image be relevant or meaningful?`;
  ...
  updateMemory(id: string, updates: Partial<MultiModalMemory>): boolean {
    ...
    if (updates.analysis !== undefined) memory.analysis = updates.analysis;
    if (updates.relatedMemories !== undefined) memory.relatedMemories = updates.relatedMemories;
