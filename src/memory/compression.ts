...
export interface ConceptNode {
  ...
  connections: Array<{ targetId: string; relation: string; weight: number }>;
  frequency: number;
  lastSeen: Date;
}

export interface CompressionConfig {
  episodeWindowSize: number;        // How many memories form an episode
  patternMinFrequency: number;      // Minimum occurrences for a pattern
  principleMinSupport: number;      // Minimum patterns to form a principle
  importanceDecayRate: number;      // How fast importance decays
  reinforcementBoost: number;       // Boost when memory is accessed
  maxClusters: number;              // Maximum concept clusters
  similarityThreshold: number;      // Cosine similarity for clustering
}
...
export class SemanticMemoryCompressor {
  ...
  private hierarchy: HierarchicalMemory;
  private conceptGraph: Map<string, ConceptNode>;
  private clusters: Map<string, MemoryCluster>;
  private memoryImportance: Map<string, { value: number; lastAccess: Date }>;
  ...
  compressToEpisodes(memories: MemoryEntry[]): CompressedEpisode[] {
    const episodes: CompressedEpisode[] = [];
   ...
   */
  private createEpisode(memories: MemoryEntry[]): CompressedEpisode {
   ...
   */
  private extractEntities(memories: MemoryEntry[]): string[] {
   ...
   */
  private calculateEmotionalValence(memories: MemoryEntry[]): number {
  ...
  extractPatterns(): ExtractedPattern[] {
    ...
    const entityCooccurrence = new Map<string, Map<string, number>>();
      ...
      for (const [e2, freq] of coMap) {
        if (freq >= this.config.patternMinFrequency) {
          const relatedEpisodes = this.hierarchy.episodes
            .filter(ep =>
              ep.keyEntities.includes(e1) && ep.keyEntities.includes(e2)
            )
            .map(ep => ep.id);
  ...
  derivePrinciples(): CorePrinciple[] {
    ...
    const patternGroups = new Map<string, ExtractedPattern[]>();
   ...
   */
  private labelCluster(memberIds: string[], memories: MemoryEntry[]): string {
   ...
   */
  private calculateClusterImportance(memberIds: string[], memories: MemoryEntry[]): number {
    const members = memories.filter(m => memberIds.includes(m.id));
   ...
   */
  reinforceMemory(memoryId: string, baseImportance: number = 0.5): void {
  ...
  private calculateStanceAlignment(memory: MemoryEntry, stance: Stance): number {
    ...
    const memoryFrame = memory.metadata?.frame as string | undefined;
   ...
   */
  getRelatedConcepts(label: string, depth: number = 2): ConceptNode[] {
  ...
  export(): {
    ...
    importance: Array<{ id: string; value: number; lastAccess: string }>;
  } {
    return {
      config: this.config,
