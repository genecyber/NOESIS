...
export interface TokenCoherence {
  token: string;
  position: number;
  localScore: number;     // Coherence with immediate context (0-1)
  globalScore: number;    // Coherence with overall conversation (0-1)
...
export interface GateResult {
  ...
  backtrackTo?: number;  // Token position to backtrack to
}
...
export interface StreamingState {
  ...
  isHealthy: boolean;
  coherenceWave: number[];  // Last N coherence scores for visualization
}
...
export interface CoherenceGateConfig {
  enabled: boolean;
  minCoherence: number;           // Minimum acceptable coherence (0-1)
  warningThreshold: number;       // Threshold to show warning (0-1)
  maxBacktracks: number;          // Maximum backtracks before termination
  windowSize: number;             // Number of tokens for moving average
  localWeight: number;            // Weight for local coherence (0-1)
  globalWeight: number;           // Weight for global coherence (0-1)
  earlyTerminationEnabled: boolean;
  visualizationEnabled: boolean;
}

const DEFAULT_CONFIG: CoherenceGateConfig = {
  enabled: true,
  minCoherence: 0.3,
...
class CoherenceGateManager {
  private config: CoherenceGateConfig = DEFAULT_CONFIG;
   ...
   */
  processToken(token: string): { coherence: TokenCoherence; gate: GateResult } {
    if (!this.state || !this.config.enabled) {
      // Disabled - pass everything through
      const coherence: TokenCoherence = {
        token,
    ...
    const combinedScore =
      localScore * this.config.localWeight +
   ...
   */
  private calculateLocalCoherence(token: string): number {
   ...
   */
  private calculateGlobalCoherence(token: string): number {
    let score = 1.0;
   ...
   * Detect coherence flags
   */
  private detectCoherenceFlags(token: string): CoherenceFlag[] {
   ...
   */
  private hasIncoherentSyntax(token: string): boolean {
   ...
   */
  private sentencesContradict(s1: string, s2: string): boolean {
   ...
   */
  private detectStanceViolation(token: string): boolean {
  ...
  private calculateTopicRelevance(text: string): number {
    if (!this.conversationContext) return 1.0;

    // Extract key terms from context
    const contextWords = new Set(
      this.conversationContext.toLowerCase()
        .split(/\s+/)
        .filter(w => w.length > 4)
    );
    ...
    if (responseWords.length === 0) return 1.0;
   ...
   * Update moving average
   */
  private updateMovingAverage(score: number): void {
   ...
   */
  private evaluateGate(coherence: TokenCoherence): GateResult {
    if (!this.state) {
   ...
   */
  private findBacktrackPoint(): number {
    if (!this.state || this.state.tokens.length === 0) return 0;
  ...
  getVisualizationData(): {
    ...
    if (!this.state || !this.config.visualizationEnabled) return null;

    // Count flags
    const flagCounts: Record<CoherenceFlag, number> = {
      topic_drift: 0,
      tone_shift: 0,
      contradiction: 0,
      repetition: 0,
  ...
  getInlineIndicator(): string {
    if (!this.state || !this.config.visualizationEnabled) return '';

    const score = this.state.movingAverage;

    if (score >= 0.8) return '\x1b[32m●\x1b[0m';  // Green
    if (score >= 0.6) return '\x1b[33m●\x1b[0m';  // Yellow
    if (score >= 0.4) return '\x1b[31m●\x1b[0m';  // Red
  ...
  finalizeStream(): {
    ...
    const result = {
      ...
      flagSummary: vis?.flagCounts || {} as Record<CoherenceFlag, number>
    };
