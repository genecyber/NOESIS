...
export class StanceAnalytics {
  private snapshots: StanceSnapshot[] = [];
  ...
  private updateMetrics(stance: Stance, coherence: number): void {
  ...
  private detectAnomalies(): void {
    ...
    const stdDev = Math.sqrt(
      coherences.reduce((sum, c) => sum + Math.pow(c - mean, 2), 0) / coherences.length
    );
  ...
  private recordAnomaly(anomaly: Omit<Anomaly, 'id' | 'detectedAt'>): void {
  ...
  private analyzeTrends(): TrendAnalysis[] {
    ...
    if (this.snapshots.length < 5) return trends;
  ...
  private calculateTrend(
    ...
    const volatility = Math.sqrt(
      data.reduce((sum, d) => sum + Math.pow(d.value - yMean, 2), 0) / n
    ) / yMean;
  ...
  private generateRecommendations(): Recommendation[] {
    ...
    const recentAnomalies = this.anomalyHistory.filter(
      a => Date.now() - a.detectedAt.getTime() < 300000
    );
  ...
  private generateForecasts(): Forecast[] {
    if (this.snapshots.length < 10) return [];
  ...
  private forecastSimple(
    ...
    const stdDev = Math.sqrt(
      data.reduce((sum, d) => sum + Math.pow(d - mean, 2), 0) / n
    );
  ...
  getUsageStatistics(): UsageStatistics {
    ...
    const monthlySessions = new Set(
      this.snapshots
        .filter(s => now - s.timestamp.getTime() < 30 * day)
        .map(s => s.sessionId)
    );

    const frameCounts = new Map<Frame, number>();
    const modelCounts = new Map<SelfModel, number>();

    for (const snapshot of this.snapshots) {
      frameCounts.set(snapshot.stance.frame,
        (frameCounts.get(snapshot.stance.frame) || 0) + 1);
      modelCounts.set(snapshot.stance.selfModel,
        (modelCounts.get(snapshot.stance.selfModel) || 0) + 1);
    }

    let mostUsedFrame: Frame = 'pragmatic';
    let maxFrameCount = 0;
    ...
    let mostUsedSelfModel: SelfModel = 'guide';
    let maxModelCount = 0;
  ...
  private dashboardToCsv(dashboard: AnalyticsDashboard): string {
    const lines: string[] = [];
  ...
  private dashboardToHtml(dashboard: AnalyticsDashboard): string {
    return `
<!DOCTYPE html>
<html>
...
export function createAnalyticsDashboard(): StanceAnalytics {
