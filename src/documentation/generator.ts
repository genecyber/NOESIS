...
const FRAME_DESCRIPTIONS: Record<Frame, string> = {
...
const SELF_MODEL_DESCRIPTIONS: Record<SelfModel, string> = {
...
export class StanceDocumentGenerator {
  ...
  private history: StanceSnapshot[] = [];
  ...
  private generateMarkdown(stance: Stance, title?: string): string {
    const lines: string[] = [];
  ...
  private generateHtml(stance: Stance, title?: string): string {
    const markdown = this.generateMarkdown(stance, title);
    return this.markdownToHtml(markdown);
  }

  private generateJson(stance: Stance): string {
...
<body>
  ...
  generateEvolutionNarrative(): EvolutionNarrative {
    ...
    return {
      ...
      timeline: events,
      keyTransformations: transformations.filter(t =>
        this.calculateSignificance(this.detectChanges(t.from as Stance, t.to as Stance)) !== 'minor'
      ),
      insights
    };
  }

  private detectChanges(prev: Stance, curr: Stance): FieldDiff[] {
    const diffs: FieldDiff[] = [];

    // Compare values
    for (const key of Object.keys(curr.values) as (keyof Values)[]) {
  ...
  private calculateSignificance(changes: FieldDiff[]): 'minor' | 'moderate' | 'major' | 'critical' {
    ...
    if (hasFrameChange && (hasSelfModelChange || hasObjectiveChange)) return 'critical';
    if (hasFrameChange || hasSelfModelChange) return 'major';

    const totalMagnitude = changes.reduce((sum, c) => sum + (c.magnitude || 0), 0);
    if (totalMagnitude > 50) return 'major';
    if (totalMagnitude > 20) return 'moderate';

    return 'minor';
  }

  private describeChanges(changes: FieldDiff[]): string {
    ...
    for (const change of changes.slice(0, 3)) {
      if (change.field === 'frame') {
        ...
        const direction = (change.value2 as number) > (change.value1 as number) ? 'increased' : 'decreased';
        parts.push(`${key} ${direction} by ${Math.abs((change.value2 as number) - (change.value1 as number))}`);
      }
    }
  ...
  private extractChangedFields(stance: Stance, changes: FieldDiff[]): Partial<Stance> {
    ...
    return extracted as Partial<Stance>;
  }
  ...
  private generateSummary(events: NarrativeEvent[]): string {
  ...
  private generateInsights(events: NarrativeEvent[], _transformations: Transformation[]): string[] {
  ...
  private analyzeTrend(): string {
    ...
    if (curiosityTrend > 10) return 'Trending toward higher curiosity and exploration';
    if (curiosityTrend < -10) return 'Trending toward lower curiosity and specialization';
  ...
  generateChangeLog(): ChangeLogEntry[] {
    ...
    for (let i = 1; i < this.history.length; i++) {
      ...
      const items: ChangeItem[] = changes.map(c => ({
  ...
  private describeChange(diff: FieldDiff): string {
    if (diff.field === 'frame') {
  ...
  generateDiffReport(stance1: Stance, stance2: Stance): DiffReport {
    ...
    for (const field of allFields) {
      ...
      if (JSON.stringify(v1) === JSON.stringify(v2)) matchingFields++;
    }
  ...
  private generateDiffSummary(differences: FieldDiff[], similarity: number): string {
  ...
  private describeValue(key: string, value: number): string {
  ...
  private getIncludes(): string[] {
    const includes: string[] = ['overview', 'values', 'sentience'];
    if (this.config.includeHistory) includes.push('history');
    if (this.config.includeRationale) includes.push('rationale');
    return includes;
  }
...
export function createDocumentGenerator(config?: Partial<DocumentationConfig>): StanceDocumentGenerator {
