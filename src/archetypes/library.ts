...
export class ArchetypeLibraryManager {
  ...
  private blends: Map<string, BlendedArchetype> = new Map();
   ...
   */
  findMatchingArchetypes(query: ContextQuery, limit: number = 5): ArchetypeMatch[] {
  ...
  private calculateMatchScore(
    ...
    // Keyword matching
    if (query.keywords) {
      const keywordMatches = query.keywords.filter(k =>
        archetype.keywords.some(ak => ak.includes(k.toLowerCase()))
      );
    ...
    if (query.intent) {
      ...
      const intentMatch = intentKeywords.some(k =>
        archetype.keywords.includes(k) ||
   ...
   */
  private suggestAdaptations(archetype: Archetype, query: ContextQuery): string[] {
  ...
  blendArchetypes(
    ...
    if (!this.config.allowBlending) return null;
    if (archetypeIds.length < 2 || archetypeIds.length > this.config.maxBlendCount) return null;

    const archetypes = archetypeIds
      .map(id => this.archetypes.get(id))
      .filter((a): a is Archetype => a !== undefined);

    if (archetypes.length !== archetypeIds.length) return null;

    // Check compatibility
    const coherenceScore = this.calculateBlendCoherence(archetypes);
    if (coherenceScore < 0.3) return null;

    // Calculate ratios if not provided
    const finalRatios: Record<string, number> = ratios || {};
    if (!ratios) {
    ...
    const blended: BlendedArchetype = {
      id: `blend-${Date.now()}`,
   ...
   */
  private calculateBlendCoherence(archetypes: Archetype[]): number {
    let coherence = 1.0;
  ...
  private blendTemplates(
    ...
    // Blend values
    const blendedValues: Partial<Stance['values']> = {};
    ...
    const blendedSentience: SentienceProfile = {
      ...
      identityStrength: Math.round(
        archetypes.reduce((sum, a) => sum + a.stanceTemplate.sentienceProfile.identityStrength * (ratios[a.id] || 0), 0)
      ),
  ...
  applyArchetype(
    ...
    if (!archetype) return currentStance;
    ...
    // Blend values
    const currentValues = currentStance.values as Record<string, number>;
    const templateValues = template.values as Record<string, number>;
    ...
    newStance.values = newValues as Stance['values'];
    ...
    newStance.sentience = {
      ...
      awarenessLevel: Math.max(
        profile.awarenessRange[0],
        Math.min(profile.awarenessRange[1], currentStance.sentience.awarenessLevel)
      ),
      autonomyLevel: Math.max(
        profile.autonomyRange[0],
        Math.min(profile.autonomyRange[1], currentStance.sentience.autonomyLevel)
      ),
      identityStrength: Math.round(
        currentStance.sentience.identityStrength * (1 - intensity * 0.3) +
