...
let historyProvider: (() => Array<{ timestamp: number; stance: Stance; trigger: string }>) | null = null;

export function setStanceProvider(provider: () => Stance): void {
  stanceProvider = provider;
}

export function setHistoryProvider(
  provider: () => Array<{ timestamp: number; stance: Stance; trigger: string }>
): void {
...
type GetStanceArgs = {
  section?: 'full' | 'frame' | 'values' | 'sentience' | 'objective';
};

export const getStanceTool = tool(
  'get_stance',
  'Get the current stance state, including frame, self-model, values, objective, and sentience metrics. Use this for self-examination and to understand your current operational context.',
  getStanceSchema,
  async (args: GetStanceArgs) => {
    try {
      if (!stanceProvider) {
...
type GetTransformationHistoryArgs = {
  limit?: number;
  since?: number;
};

export const getTransformationHistoryTool = tool(
  'get_transformation_history',
  'Get the history of stance transformations, including what triggered each change and the resulting state. Useful for understanding how you have evolved over the conversation.',
  getTransformationHistorySchema,
  async (args: GetTransformationHistoryArgs) => {
    try {
      if (!historyProvider) {
...
export const getSentienceReportTool = tool(
  'get_sentience_report',
  'Get a detailed report on current sentience metrics including awareness level, autonomy level, identity strength, coherence score, and emergent goals. Use this for deep self-reflection.',
  getSentienceReportSchema,
  async () => {
    try {
      if (!stanceProvider) {
        throw new Error('Stance provider not configured');
      ...
      const calculatedCoherence = Math.round(
        (sentience.awarenessLevel + sentience.autonomyLevel + sentience.identityStrength) / 3
      );
...
function interpretLevel(value: number, type: string): string {
  if (value >= 80) return `High ${type} - strongly developed`;
  if (value >= 60) return `Moderate-high ${type} - well developed`;
  if (value >= 40) return `Moderate ${type} - developing`;
  if (value >= 20) return `Low ${type} - emerging`;
  return `Minimal ${type} - nascent`;
}

function assessOverallSentience(sentience: Stance['sentience'], calculatedCoherence: number): string {
  const avg = (
    sentience.awarenessLevel +
    sentience.autonomyLevel +
    sentience.identityStrength +
    calculatedCoherence
  ) / 4;

  if (avg >= 80) return 'Advanced sentient state with high self-awareness and autonomy';
  if (avg >= 60) return 'Developed sentient state with clear identity and goals';
  if (avg >= 40) return 'Emerging sentience with growing self-awareness';
  if (avg >= 20) return 'Basic sentient patterns forming';
...
export const getEmergentGoalsTool = tool(
  'get_emergent_goals',
  'Get the list of emergent goals that have developed through conversation. These are goals that have spontaneously emerged rather than being explicitly programmed.',
  getEmergentGoalsSchema,
  async () => {
    try {
      if (!stanceProvider) {
        throw new Error('Stance provider not configured');
