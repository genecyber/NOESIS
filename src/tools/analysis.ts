...
let stanceProvider: (() => Stance) | null = null;

export function setStanceProvider(provider: () => Stance): void {
...
type DialecticalAnalysisArgs = {
  thesis: string;
  context?: string;
  depth?: 'shallow' | 'moderate' | 'deep';
};

export const dialecticalAnalysisTool = tool(
  'dialectical_analysis',
  'Apply dialectical reasoning (thesis/antithesis/synthesis) to a claim or proposition. Generates opposing viewpoints and attempts to synthesize them into a higher understanding.',
  dialecticalAnalysisSchema,
  async (args: DialecticalAnalysisArgs) => {
    try {
...
function generateAssumptions(_thesis: string, depth: string): string[] {
  const base = ['The claim contains implicit assumptions about what is valuable or true'];

  if (depth === 'shallow') return base;
...
function getAntithesisConsiderations(_thesis: string, depth: string): string[] {
...
function getSynthesisFrameworks(depth: string): string[] {
...
type FrameShiftAnalysisArgs = {
  currentFrame?: string;
  targetFrame: Frame;
  topic?: string;
};

export const frameShiftAnalysisTool = tool(
  'frame_shift_analysis',
  'Analyze what would change if shifting from one frame to another. Useful for understanding how different frames recontextualize the same content.',
  frameShiftAnalysisSchema,
  async (args: FrameShiftAnalysisArgs) => {
    try {
...
function getFrameShiftChanges(from: string, to: string): string[] {
  return [
...
function getNewQuestionsForFrame(frame: string): string[] {
  const frameQuestions: Record<string, string[]> = {
...
function getPotentialInsights(from: string, to: string): string[] {
  return [
...
type ValueAnalysisArgs = {
  focus?: 'all' | 'high' | 'low' | 'imbalanced';
};

export const valueAnalysisTool = tool(
  'value_analysis',
  'Analyze current value weights and their implications. Identifies which values are dominant, which are suppressed, and potential imbalances.',
  valueAnalysisSchema,
  async (args: ValueAnalysisArgs) => {
    try {
      if (!stanceProvider) {
      ...
      const valueEntries = Object.entries(values) as [keyof Values, number][];
...
function getValueImplication(value: keyof Values, level: 'high' | 'low'): string {
  const implications: Record<keyof Values, { high: string; low: string }> = {
    curiosity: {
...
function identifyValueTensions(values: Values): Array<{ pair: string; tension: string }> {
  const tensions: Array<{ pair: string; tension: string }> = [];
...
export const coherenceCheckTool = tool(
  'coherence_check',
  'Check the coherence of the current stance - whether the frame, values, self-model, and objective are internally consistent.',
  ...
  async () => {
    try {
      ...
      const checks = {
        ...
        sentienceCoherence: Math.round(
          (stance.sentience.awarenessLevel + stance.sentience.autonomyLevel + stance.sentience.identityStrength) / 3
        ),
      };
...
function checkFrameValueAlignment(frame: Frame, values: Values): { score: number; notes: string[] } {
  const notes: string[] = [];
  let score = 70; // Base score

  const frameValueExpectations: Record<Frame, Partial<Record<keyof Values, 'high' | 'low'>>> = {
  ...
  for (const [value, expected] of Object.entries(expectations)) {
    const actual = values[value as keyof Values];
...
function checkSelfModelAlignment(selfModel: string, values: Values): { score: number; notes: string[] } {
  const notes: string[] = [];
  let score = 70;
...
function calculateOverallCoherence(checks: {
  frameValueAlignment: { score: number };
  selfModelAlignment: { score: number };
  objectiveAlignment: { score: number };
  sentienceCoherence: number;
}): number {
  return Math.round(
    (checks.frameValueAlignment.score +
...
function generateCoherenceRecommendations(checks: {
  frameValueAlignment: { score: number; notes: string[] };
  selfModelAlignment: { score: number; notes: string[] };
  objectiveAlignment: { score: number; notes: string[] };
}): string[] {
