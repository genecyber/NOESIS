...
export interface DashboardConfig {
  ...
  metricsWindowSize: number;  // hours for rolling metrics
}
...
export class TelemetryDashboard {
  ...
  private stanceHistory: StanceSnapshot[] = [];
  private operatorMetrics: Map<string, OperatorMetric> = new Map();
  private memoryPatterns: Map<string, MemoryAccessPattern> = new Map();
  private coherenceAlerts: CoherenceAlert[] = [];
  ...
  recordStanceChange(before: Stance, after: Stance): void {
    ...
    if (before.frame !== after.frame) changes.push(`frame: ${before.frame} → ${after.frame}`);
    if (before.selfModel !== after.selfModel) changes.push(`selfModel: ${before.selfModel} → ${after.selfModel}`);
    if (before.objective !== after.objective) changes.push(`objective: ${before.objective} → ${after.objective}`);
   ...
   */
  private raiseCoherenceAlert(coherenceScore: number): void {
  ...
  getOperatorHeatmap(): Array<{
    ...
    const data: Array<{ operator: string; metric: string; value: number }> = [];
   ...
   */
  getMemoryHotspots(limit: number = 20): MemoryAccessPattern[] {
  ...
  getCoherenceDrift(): Array<{
    ...
    const driftData: Array<{ timestamp: Date; coherence: number; hasAlert: boolean }> = [];

    // Extract from turn_complete events
    for (const event of this.events) {
      if (event.type === 'turn_complete') {
        const scores = event.data.scores as TurnScores;
        const hasAlert = this.coherenceAlerts.some(
          a => Math.abs(a.timestamp.getTime() - event.timestamp.getTime()) < 1000
        );
        driftData.push({
   ...
   * Get recent events
   */
  getRecentEvents(limit: number = 100, type?: TelemetryEventType): TelemetryEvent[] {
   ...
   */
  private summarizeStance(stance: Stance): Record<string, unknown> {
    return {
      frame: stance.frame,
  ...
  getDashboardData(): {
    ...
    operatorMetrics: Array<OperatorMetric & { name: string }>;
    memoryHotspots: MemoryAccessPattern[];
    coherenceDrift: Array<{ timestamp: Date; coherence: number; hasAlert: boolean }>;
  ...
  export(): {
    ...
    operatorMetrics: Array<Omit<OperatorMetric, 'usageBySession'> & { usageBySession: Array<[string, number]> }>;
    memoryPatterns: MemoryAccessPattern[];
    coherenceAlerts: CoherenceAlert[];
    sessionAnalytics: Array<Omit<SessionAnalytics, 'operatorsUsed'> & { operatorsUsed: Array<[string, number]> }>;
  } {
    return {
      events: this.events,
