...
export class PromptRewriter {
  ...
  private frameStyles: Map<Frame, FrameStyle> = new Map();
  private phraseMappings: PhraseMapping[] = [];
   ...
   */
  private initializeFrameStyles(): void {
   ...
   */
  private initializePhraseMappings(): void {
    this.phraseMappings = [
      {
        generic: 'explain',
        framed: {
   ...
   */
  private hasFrameIndicator(prompt: string, frame: Frame): boolean {
    const style = this.frameStyles.get(frame);
   ...
   */
  private calculateCoherenceScore(prompt: string, stance: Stance): number {
   ...
   */
  private calculateStanceAlignment(prompt: string, stance: Stance): number {
    let alignment = 0.5;
    ...
    if (style) {
      const promptLower = prompt.toLowerCase();
      const vocabMatches = style.vocabulary.filter(
        v => promptLower.includes(v.toLowerCase())
      ).length;
  ...
  private updateStats(
    ...
    this.stats.averageCoherenceImprovement = (
      this.stats.averageCoherenceImprovement * prevCount + coherenceScore
    ) / this.stats.promptsRewritten;

    this.stats.averageStanceAlignment = (
      this.stats.averageStanceAlignment * prevCount + stanceAlignment
    ) / this.stats.promptsRewritten;
