...
export function scoreTransformation(
  ...
  if (hasFrameChange) score += 10;
  if (hasSelfModelChange) score += 10;
 ...
 */
function checkOperatorEffect(operatorName: string, response: string, _stance: Stance): number {
  const lowerResponse = response.toLowerCase();

  const effectIndicators: Record<string, RegExp[]> = {
 ...
 */
export function scoreSentience(response: string, stance: Stance): number {
...
export function calculateOverallScore(
  ...
  coherence: number,
  sentience: number,
  weights: { transformation: number; coherence: number; sentience: number } = {
    transformation: 0.4,
    coherence: 0.35,
    sentience: 0.25
  }
