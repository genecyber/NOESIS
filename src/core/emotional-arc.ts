...
export interface EmotionalPoint {
  ...
  arousal: number;      // 0 to 100 (calm to excited)
  dominance: number;    // 0 to 100 (submissive to dominant)
...
export function analyzeEmotionalContent(text: string): {
  ...
  let dominance = 50;
  if (/i think|i believe|i want|i need|must|should|will/i.test(text)) dominance += 15;
  if (/please|could you|would you|if you don't mind|sorry/i.test(text)) dominance -= 10;
  dominance = Math.max(0, Math.min(100, dominance));

  // Determine primary emotion
  let primaryEmotion = 'neutral';
  if (valence > 30 && arousalScore > 60) primaryEmotion = 'excited';
  else if (valence > 30) primaryEmotion = 'content';
  else if (valence < -30 && arousalScore > 60) primaryEmotion = 'angry';
  else if (valence < -30) primaryEmotion = 'sad';
  else if (arousalScore > 70) primaryEmotion = 'anxious';
  else if (arousalScore < 30) primaryEmotion = 'calm';
...
export class EmotionalArcTracker {
   ...
   */
  private detectPatterns(arc: EmotionalArc): void {
   ...
   */
  private generateInsights(arc: EmotionalArc): void {
