...
export interface OperatorStrategy {
  ...
  description: string;
  steps: OperatorName[];
  triggers: TriggerType[];  // Triggers that activate this strategy
  minIntensity: number;     // Minimum intensity to activate
  cooldownTurns: number;    // Turns to wait after completion before reactivating
}

/**
 * Predefined multi-turn strategies
 */
export const OPERATOR_STRATEGIES: OperatorStrategy[] = [
  {
...
class StrategyManager {
   ...
   */
  private completeStrategy(conversationId: string): void {
    const state = this.activeStrategies.get(conversationId);
    if (!state) return;
  ...
  selectStrategy(
    ...
    _stance: Stance,  // Reserved for future stance-based strategy selection
    intensity: number
    ...
    const candidates = OPERATOR_STRATEGIES.filter(strategy => {
      ...
      if (intensity < strategy.minIntensity) return false;

      // Check cooldown
      if (this.isInCooldown(conversationId, strategy.name)) return false;
 ...
 */
export function getStrategy(name: string): OperatorStrategy | undefined {
