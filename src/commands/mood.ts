...
import type { EmotionalPattern } from '../core/emotional-arc.js';

export const moodCommand: CommandDefinition = {
  name: 'mood',
  ...
  execute(context: CommandContext, args: string[]): CommandResult {
    ...
    const lines: string[] = ['Emotional Arc Analysis:'];
...
function getPatternIcon(pattern: EmotionalPattern['type'] | null): string {
  switch (pattern) {
...
function formatBar(value: number, width: number = 10): string {
