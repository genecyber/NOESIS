...
export class StanceFingerprinter {
  private fingerprints: Map<string, StanceFingerprint> = new Map();
  ...
  generateFingerprint(stance: Stance, source?: string): StanceFingerprint {
    ...
    const fingerprint: StanceFingerprint = {
  ...
  private computeComponents(stance: Stance): FingerprintComponents {
  ...
  private combineHashes(components: FingerprintComponents): string {
  ...
  private additionalHash(input: string): string {
    let hash = 5381;
    for (let i = 0; i < input.length; i++) {
  ...
  private calculateUniqueness(components: FingerprintComponents): number {
    const existingFingerprints = Array.from(this.fingerprints.values());

    if (existingFingerprints.length === 0) return 100;

    const similarities = existingFingerprints.map(fp =>
      this.calculateComponentSimilarity(components, fp.components)
    );
  ...
  private calculateComponentSimilarity(
    ...
    if (a.frameHash === b.frameHash) totalSimilarity += COMPONENT_WEIGHTS.frame;
    if (a.valuesHash === b.valuesHash) totalSimilarity += COMPONENT_WEIGHTS.values;
    if (a.selfModelHash === b.selfModelHash) totalSimilarity += COMPONENT_WEIGHTS.selfModel;
    if (a.objectiveHash === b.objectiveHash) totalSimilarity += COMPONENT_WEIGHTS.objective;
    if (a.sentienceHash === b.sentienceHash) totalSimilarity += COMPONENT_WEIGHTS.sentience;

    return totalSimilarity;
  }

  private resolveCollision(hash: string): CollisionResolution {
    let attempts = 0;
  ...
  compareFingerprints(fp1: string, fp2: string): SimilarityResult {
    ...
    const componentSimilarities: ComponentSimilarity[] = [
      {
        component: 'frame',
    ...
    const similarity = componentSimilarities.reduce(
      (sum, cs) => sum + cs.similarity * cs.weight,
      0
    );
  ...
  findDuplicates(stance: Stance): DuplicateMatch[] {
    ...
    for (const fp of this.fingerprints.values()) {
      ...
      if (similarity >= this.config.duplicateThreshold) {
        ...
        if (tempComponents.frameHash === fp.components.frameHash) matchedComponents.push('frame');
        if (tempComponents.valuesHash === fp.components.valuesHash) matchedComponents.push('values');
        if (tempComponents.selfModelHash === fp.components.selfModelHash) matchedComponents.push('selfModel');
        if (tempComponents.objectiveHash === fp.components.objectiveHash) matchedComponents.push('objective');
        if (tempComponents.sentienceHash === fp.components.sentienceHash) matchedComponents.push('sentience');
  ...
  findByHash(hash: string): StanceFingerprint | undefined {
    return Array.from(this.fingerprints.values()).find(fp =>
      fp.hash === hash || fp.shortHash === hash
    );
  }

  findSimilar(fingerprintId: string, threshold: number = 0.7): StanceFingerprint[] {
    const target = this.fingerprints.get(fingerprintId);
    if (!target) return [];

    const similar: Array<{ fp: StanceFingerprint; similarity: number }> = [];

    for (const fp of this.fingerprints.values()) {
      if (fp.id === fingerprintId) continue;
  ...
  getUniquenessDistribution(): { bucket: string; count: number }[] {
    const buckets: Record<string, number> = {
      '0-20': 0, '21-40': 0, '41-60': 0, '61-80': 0, '81-100': 0
    };

    for (const fp of this.fingerprints.values()) {
      if (fp.uniquenessScore <= 20) buckets['0-20']++;
      else if (fp.uniquenessScore <= 40) buckets['21-40']++;
      else if (fp.uniquenessScore <= 60) buckets['41-60']++;
      else if (fp.uniquenessScore <= 80) buckets['61-80']++;
      else buckets['81-100']++;
    }
...
export function createFingerprinter(config?: Partial<FingerprintConfig>): StanceFingerprinter {
