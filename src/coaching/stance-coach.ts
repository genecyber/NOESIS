...
const FRAME_SYNERGIES: Record<Frame, Frame[]> = {
...
const VALUE_DESCRIPTIONS: Record<keyof Values, { low: string; high: string; optimal: string }> = {
  curiosity: {
...
export class StanceCoach {
  ...
  private stanceHistory: Map<string, StanceProfile[]> = new Map();
  ...
  private generateMilestones(current: number, target: number): Milestone[] {
  ...
  analyzeWeaknesses(sessionId: string, stance: Stance): WeaknessAnalysis[] {
    ...
    // Analyze values
    const valueKeys = Object.keys(stance.values) as (keyof Values)[];
  ...
  private updateRecommendations(sessionId: string): void {
    const history = this.stanceHistory.get(sessionId);
  ...
  assignExercise(sessionId: string, exerciseId?: string): Exercise | null {
    ...
    if (exerciseId) {
      ...
      const suitable = this.exerciseLibrary.filter(e =>
        e.difficulty === difficulty && !session.exercises.some(se => se.id === e.id && se.completed)
      );
  ...
  completeExercise(sessionId: string, exerciseId: string, feedback?: ExerciseFeedback): boolean {
    ...
    if (exercise.targetSkill && session.progress.skillLevels[exercise.targetSkill] !== undefined) {
      ...
      session.progress.skillLevels[exercise.targetSkill] = Math.min(100,
        session.progress.skillLevels[exercise.targetSkill] + improvement
      );
    }
  ...
  private grantAchievement(session: CoachingSession, achievement: Achievement): void {
    if (!session.progress.achievements.some(a => a.id === achievement.id)) {
      session.progress.achievements.push(achievement);
    }
  }

  private updateProgress(session: CoachingSession): void {
...
export function createStanceCoach(): StanceCoach {
