...
class CollaborativeSessionManager {
    sessions = new Map();
    codeToSession = new Map(); // code -> session ID
     ...
     */
    createSession(hostName, stance, options = {}) {
     ...
     */
    joinSession(code, participantName, asObserver = false) {
     ...
     */
    addMessage(sessionId, participantId, content, isAgentResponse = false) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return null;
        const participant = session.participants.get(participantId);
     ...
     */
    castVote(sessionId, participantId, operatorName) {
        const session = this.sessions.get(sessionId);
        if (!session || !session.config.votingEnabled)
            return null;
        const participant = session.participants.get(participantId);
        ...
        if (!currentMessage || currentMessage.isAgentResponse)
            return null;
        ...
        // Add new vote
        const vote = { participantId, operatorName, weight };
     ...
     */
    getWinningOperator(sessionId) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return null;
        const currentMessage = session.messages[session.messages.length - 1];
        if (!currentMessage?.votes || currentMessage.votes.length === 0)
            return null;
     ...
     */
    requestTurn(sessionId, participantId) {
        const session = this.sessions.get(sessionId);
        if (!session || session.mode !== 'turn-taking')
            return -1;
        const participant = session.participants.get(participantId);
        if (!participant || participant.role === 'observer')
            return -1;
     ...
     */
    broadcastSpeakerChange(session) {
    ...
    stopRecording(sessionId) {
        const session = this.sessions.get(sessionId);
        if (!session || !session.isRecording || !session.recording)
            return null;
     ...
     */
    setTypingStatus(sessionId, participantId, isTyping) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return;
        const participant = session.participants.get(participantId);
