...
class MemoryPersistenceManager {
    ...
    memoryHashes = new Map(); // id -> content hash
    /**
     * Set configuration
     */
    setConfig(config) {
     ...
     */
    async exportMemories(memories, filepath, format) {
     ...
     */
    async createBackup(memories, stance, trigger) {
        this.ensureBackupDir();
    ...
    pruneBackups() {
        ...
        const sorted = [...this.backups].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
    ...
    shouldAutoBackup(currentDrift) {
        ...
        if (this.config.autoBackupInterval > 0) {
            const hoursSinceBackup = (Date.now() - this.lastBackupTime.getTime()) / (1000 * 60 * 60);
     ...
     */
    getDeduplicationRemovals(memories) {
        const duplicates = this.findDuplicates(memories);
        const idsToRemove = [];
        for (const [_hash, ids] of duplicates) {
            const sorted = ids
        ...
        for (const [_type, typeMemories] of byType) {
            for (let i = 0; i < typeMemories.length; i++) {
                for (let j = i + 1; j < typeMemories.length; j++) {
                    const similarity = this.calculateSimilarity(typeMemories[i].content, typeMemories[j].content);
                    if (similarity >= this.config.deduplicationThreshold) {
                        // Boost importance of the kept memory
                        if (typeMemories[i].importance < 1.0) {
                            typeMemories[i].importance = Math.min(1.0, typeMemories[i].importance + 0.1);
                            importanceBoosts++;
                        }
    ...
    calculateStats(memories) {
        ...
        const healthScore = Math.round((avgImportance * 40) +
            ((1 - duplicateRatio) * 30) +
            (typeBalance * 30));
        return {
