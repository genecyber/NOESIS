...
export class MemoryPrioritizationManager {
     ...
     */
    addMemory(content, type, stance, metadata = {}) {
        const now = new Date();
     ...
     */
    calculateSalience(content, stance) {
     ...
     */
    getFrameRelevance(content, frame) {
        const frameKeywords = {
     ...
     */
    calculateInitialPriority(salience, alignment) {
     ...
     */
    predictForgetting(memory) {
        if (memory.priority >= 80)
            return null; // High priority memories don't get forgotten
    ...
    retrieve(query, currentStance) {
        ...
        for (const memory of this.memories.values()) {
            ...
            if (memory.decay.currentStrength < 0.1)
                continue;
            // Filter by type
            if (query.type && memory.type !== query.type)
                continue;
            // Filter by priority
            if (query.minPriority && memory.priority < query.minPriority)
                continue;
            // Filter by age
            if (query.maxAge) {
            ...
            // Text search
            if (query.text && !memory.content.toLowerCase().includes(query.text.toLowerCase()))
                continue;
            results.push(memory);
        }
     ...
     */
    calculateStanceBoost(stance, memories) {
    ...
    calculateSimilarity(mem1, mem2) {
        ...
        const sharedFrames = mem1.stanceAlignment.alignedFrames.filter(f => mem2.stanceAlignment.alignedFrames.includes(f)).length;
     ...
     */
    forgetLowPriority(threshold = 20) {
        const events = [];
     ...
     */
    updatePriority(memoryId, newPriority) {
     ...
     * Update averages
     */
    updateAverages(newMemory) {
        const n = this.stats.activeMemories;
        this.stats.averagePriority = (this.stats.averagePriority * (n - 1) + newMemory.priority) / n;
        this.stats.averageSalience = (this.stats.averageSalience * (n - 1) + newMemory.salience.overall) / n;
    }
    /**
     * List all memories
     */
    listMemories(sortBy = 'priority') {
        const memories = [...this.memories.values()];
        switch (sortBy) {
            case 'priority':
                return memories.sort((a, b) => b.priority - a.priority);
            case 'recency':
                return memories.sort((a, b) => b.decay.lastAccessed.getTime() - a.decay.lastAccessed.getTime());
            case 'strength':
