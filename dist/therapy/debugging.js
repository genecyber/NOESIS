...
const COHERENCE_RULES = [
    ...
    },
    {
        ...
        check: (s) => {
            ...
            if (s.objective === 'self-actualization' && s.sentience.autonomyLevel < 30)
                return false;
            if (s.objective === 'helpfulness' && s.sentience.autonomyLevel > 90)
                return false;
            return true;
        },
        severity: 'medium',
...
export class StanceTherapist {
    ...
    takeSnapshot(stance, context, event) {
        this.snapshots.push({
            timestamp: new Date(),
    ...
    applyIntervention(sessionId, type, target, newValue) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return false;
        const before = session.currentStance[target];
        session.currentStance[target] = newValue;
    ...
    completeSession(sessionId) {
        ...
        else if (finalHealth > initialHealth)
    ...
    findApplicablePlaybook(_stance) {
    ...
    executePlaybook(playbook, stance) {
        let result = { ...stance };
    ...
    executePlaybookStep(step, stance) {
        switch (step.action) {
            case 'normalize-sentience':
                const target = step.params.target || 50;
                return {
                    ...stance,
                    sentience: {
                        ...stance.sentience,
