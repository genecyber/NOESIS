...
const FRAME_OBJECTIVE_COMPATIBILITY = {
...
function countValueChanges(before, after) {
    let changes = 0;
    const threshold = 20;
    if (Math.abs(before.curiosity - after.curiosity) > threshold)
        changes++;
    if (Math.abs(before.certainty - after.certainty) > threshold)
        changes++;
    if (Math.abs(before.risk - after.risk) > threshold)
        changes++;
    if (Math.abs(before.novelty - after.novelty) > threshold)
        changes++;
    if (Math.abs(before.empathy - after.empathy) > threshold)
        changes++;
    if (Math.abs(before.provocation - after.provocation) > threshold)
        changes++;
    if (Math.abs(before.synthesis - after.synthesis) > threshold)
        changes++;
    return changes;
}
export class StanceSimulator {
    simulations = new Map();
    comparisons = new Map();
    simulate(currentStance, proposedChanges) {
    ...
    assessRiskLevel(coherenceDelta, breakingChanges) {
        ...
        if (errorCount >= 2 || coherenceDelta < -30)
            return 'critical';
        if (errorCount === 1 || coherenceDelta < -20)
            return 'high';
        if (warningCount >= 2 || coherenceDelta < -10)
            return 'medium';
        if (warningCount === 1 || coherenceDelta < 0)
            return 'low';
        return 'none';
    }
    predictSideEffects(before, after) {
        const effects = [];
    ...
    generateRollbackScenarios(original) {
        return [
            {
                name: 'Full Rollback',
    ...
    generateRecommendation(coherenceImpact, sideEffects) {
    ...
    compareStances(stanceA, stanceB) {
    ...
    generateComparisonScenarios(stanceA, stanceB) {
        return [
    ...
    evaluateScenario(name, stanceA, stanceB, scorer) {
    ...
    generateComparisonSummary(stanceA, stanceB, winner, scoreA, scoreB) {
        if (winner === 'tie') {
