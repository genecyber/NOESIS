...
export class OperatorPredictionManager {
     ...
     */
    recordTurn(stance, operator, message) {
    ...
    detectPatterns() {
        ...
        const isEscalating = driftDeltas.every((d, i) => i === 0 || d >= driftDeltas[i - 1] - 1);
     ...
     */
    detectInflectionPoints(currentTurn) {
     ...
     */
    predictNextOperator(currentStance, lastMessage) {
     ...
     * Predict from intent
     */
    predictFromIntent(intent) {
        const intentOperatorMap = {
            explore: 'EXPLORE',
            challenge: 'CHALLENGE',
            understand: 'DEEPEN',
            create: 'CREATE',
     ...
     */
    determineUrgency(prediction, stance) {
     ...
     */
    determineCategory(prediction) {
        const proactive = ['DEEPEN', 'EXPLORE', 'CREATE', 'PLAY'];
        const reactive = ['STABILIZE', 'RESOLVE', 'DIVERSIFY'];
        if (proactive.includes(prediction.operator))
            return 'proactive';
        if (reactive.includes(prediction.operator))
            return 'reactive';
        return 'exploratory';
    }
    /**
     ...
     */
    applyNoveltyBalance(suggestions) {
        // Boost rarely used operators
        const recentOperators = this.operatorHistory.slice(-10);
        for (const suggestion of suggestions) {
            const recentUses = recentOperators.filter(op => op === suggestion.prediction.operator).length;
            if (recentUses === 0) {
     ...
     */
    calculateOptimalPath(currentStance, targetFrame, maxTurns = 5) {
        const operators = [];
        ...
        for (let turn = 0; turn < maxTurns; turn++) {
            if (simulatedStance.frame === targetFrame)
                break;
     ...
     */
    validatePrediction(actualOperator) {
