...
export class ConversationStanceInference {
    ...
    buildStanceFromEvidence(evidence) {
        const values = createDefaultValues();
        let frame = 'pragmatic';
        let selfModel = 'guide';
        let objective = 'helpfulness';
        for (const ev of evidence) {
            if (ev.confidence < this.config.minConfidence)
                continue;
            if (ev.field === 'frame') {
                frame = ev.value;
            }
...
export function createStanceInference(config) {
