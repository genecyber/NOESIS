...
export class PersonalityMarketplace extends EventEmitter {
    ...
    categories = new Map();
    bundles = new Map();
    userPresets = new Map(); // userId -> presetIds
    installedPresets = new Set();
    constructor() {
        super();
    ...
    createPresetFromTemplate(id, template, author) {
        return {
            id,
    ...
    updatePreset(presetId, updates, changeLog) {
    ...
    deletePreset(presetId, userId) {
        const preset = this.presets.get(presetId);
        if (!preset || preset.author !== userId)
            return false;
    ...
    getSimilarPresets(presetId, limit = 5) {
        const preset = this.presets.get(presetId);
        if (!preset)
            return [];
        return Array.from(this.presets.values())
    ...
    uninstallPreset(presetId) {
        if (!this.installedPresets.has(presetId))
            return false;
    ...
    // Reviews & Ratings
    // ============================================================================
    addReview(presetId, userId, rating, comment) {
        const preset = this.presets.get(presetId);
        if (!preset)
            return null;
    ...
    markReviewHelpful(reviewId, presetId) {
        const reviews = this.reviews.get(presetId);
        if (!reviews)
            return false;
    ...
    importPreset(jsonData, newAuthor) {
        try {
    ...
    // Version Control
    // ============================================================================
    getPresetVersions(presetId) {
        return this.versions.get(presetId) || [];
    }
    revertToVersion(presetId, version) {
        ...
        if (!targetVersion)
            return null;
        const reverted = this.updatePreset(presetId, targetVersion.preset, `Reverted to version ${version}`);
        if (reverted) {
