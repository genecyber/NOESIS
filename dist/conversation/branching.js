...
class ConversationBranchManager {
     ...
     */
    initializeRoot(messages, stance, config, name = 'main') {
     ...
     * Create a new branch
     */
    createBranch(name, parentBranchId, branchPoint, messages, stance, config) {
        const id = uuidv4();
        const branch = {
            id,
            name,
     ...
     */
    branchAt(messageIndex, name, reason) {
        ...
        const newBranch = this.createBranch(name, this.activeBranchId, branchPoint, branchedMessages, stanceAtPoint, activeBranch.config);
        return newBranch;
    }
    /**
     * Branch from the current point
     */
    branchNow(name, reason) {
        if (!this.activeBranchId)
            return null;
        const activeBranch = this.branches.get(this.activeBranchId);
        if (!activeBranch)
     ...
     */
    timeTravelTo(branchId, messageIndex) {
        const branch = this.branches.get(branchId);
     ...
     */
    restoreFromSnapshot(snapshotId, branchName) {
        ...
        if (!originalBranch)
            return null;
        return this.branchAt(snapshot.messageIndex, branchName, `Restored from snapshot at message ${snapshot.messageIndex}`);
    }
    /**
    ...
    compareBranches(branchId1, branchId2) {
        ...
        const divergencePoint = branch1.branchPoint?.createdAt ||
            ...
            new Date(Math.max(branch1.createdAt.getTime(), branch2.createdAt.getTime()));
        return {
            branch1,
            branch2,
            commonAncestorIndex,
     ...
     */
    findCommonAncestorIndex(branch1, branch2) {
     ...
     * Merge two branches
     */
    mergeBranches(targetBranchId, sourceBranchId, conflictResolutions) {
        ...
        for (const key of Object.keys(mergedValues)) {
            const v1 = targetBranch.stance.values[key];
            const v2 = sourceBranch.stance.values[key];
            if (v1 !== v2) {
                mergedValues[key] = Math.round((v1 + v2) / 2);
                autoResolved++;
            }
        }
        ...
        const mergedBranch = this.createBranch(`${targetBranch.name}+${sourceBranch.name}`, targetBranchId, null, mergedMessages, mergedStance, targetBranch.config);
        return {
            success: true,
            mergedBranch,
            conflicts,
    ...
    archiveBranch(branchId) {
        const branch = this.branches.get(branchId);
        if (!branch || branchId === this.rootBranchId)
            return false;
