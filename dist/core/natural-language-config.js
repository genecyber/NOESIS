...
class NaturalLanguageConfigManager {
     ...
     */
    calculateConfidence(changes, text) {
        if (changes.length === 0)
            return 0;
     ...
     */
    generatePreview(intent, currentStance, currentConfig) {
        ...
        for (const change of intent.changes) {
            ...
            switch (change.target) {
                ...
                case 'frame':
                    if (change.key) {
                        proposedStance.frame = change.key;
                        stanceDelta.frame = change.key;
                    }
                    break;
                case 'selfModel':
                    if (change.key) {
                        proposedStance.selfModel = change.key;
                        stanceDelta.selfModel = change.key;
                    }
                    break;
                case 'intensity':
     ...
     */
    applyConfiguration(intent, stance, config) {
        ...
        if (preview.stanceDelta.frame)
            stance.frame = preview.stanceDelta.frame;
        if (preview.stanceDelta.selfModel)
            stance.selfModel = preview.stanceDelta.selfModel;
        if (preview.stanceDelta.values) {
            for (const [key, value] of Object.entries(preview.stanceDelta.values)) {
                stance.values[key] = value;
            }
        }
        // Apply config changes
        if (preview.proposedValues.intensity !== undefined) {
            config.intensity = preview.proposedValues.intensity;
        }
        // Record in history
        const entry = {
            id: `cfg_${Date.now()}`,
     ...
     */
    savePreset(name, description, stance, config) {
        const preset = {
            name,
            description,
            stance: {
...
//# sourceMappingURL=natural-language-config.js.map