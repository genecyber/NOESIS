...
/**
 * POST /api/idle-mode/session/pause - Pause active autonomous session
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await backendFetch(request, '/api/idle-mode/session/pause', {
      method: 'POST',
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        return NextResponse.json({
          success: true,
          message: 'Session would be paused once the backend implements the autonomous idle system'
        });
      }
    ...
    console.error('Failed to pause autonomous session:', error);
    return NextResponse.json(
